...
import br.univel.model.Endereco;
...
public class EnderecoEndpoint
{
   ...
   {
      em.persist(entity);
      return Response.created(UriBuilder.fromResource(EnderecoEndpoint.class).path(String.valueOf(entity.getId())).build()).build();
   }
   ...
   {
      TypedQuery<Endereco> findByIdQuery = em.createQuery("SELECT DISTINCT e FROM Endereco e WHERE e.id = :entityId ORDER BY e.id", Endereco.class);
   ...
   {
      TypedQuery<Endereco> findAllQuery = em.createQuery("SELECT DISTINCT e FROM Endereco e ORDER BY e.id", Endereco.class);
